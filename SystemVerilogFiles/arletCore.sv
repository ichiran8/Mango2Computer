...
logic  [7:0] AXYS[3:0];   // A, X, Y and S register file

logic  C = 0;             // carry flag (init at zero to avoid X's in ALU sim)
logic  Z = 0;             // zero flag
logic  I = 0;             // interrupt flag
logic  D = 0;             // decimal flag
logic  V = 0;             // overflow flag
logic  N = 0;             // negative flag
...
// wire [7:0] data_in;          // Data In
wire [7:0] IR;          // Instruction register
// logic  [7:0] data_out;          // Data Out 
...
logic NMI_edge = 0;       // captured NMI edge
...
typedef enum logic [5:0] {
    ...
    DECODE = 6'd12, // IR is valid, decode instruction, and write prev logic
    ...
    REG    = 6'd36, // Read register for logic-logic transfers
...
logic [1:0] src_reg;      // source register index
logic [1:0] dst_reg;      // destination register index

logic index_y;            // if set, then Y is index logic rather than X 
...
logic [3:0] op;           // Main ALU operation for instruction
...
always_comb begin
    case( state )
        ...
        JMPI1,
        RTI4,
        ABS1:           address_bus = { DIMUX, ADD };

        BRA2,
        INDY3,
        ABSX2:          address_bus = { ADD, ABL };

        BRA1:           address_bus = { ABH, ADD };

        JSR0,
        PUSH1,
        RTS0,
        RTI0,
        BRK0:           address_bus = { STACKPAGE, regfile };
        ...
        RTI2,
        RTI3,
        BRK2:           address_bus = { STACKPAGE, ADD };
        
        INDY1,
        INDX1,
        ZPX1,
        INDX2:          address_bus = { ZEROPAGE, ADD };

        ZP0,
        INDY0:          address_bus = { ZEROPAGE, DIMUX };

        REG,
        READ,
        WRITE:          address_bus = { ABH, ABL };
 ...
 * source of the address, such as the ALU or data_in.
 */
...
always_comb begin
    case( state )
        ...
        PUSH1:   data_out = php ? P : ADD;

        BRK2:    data_out = (IRQ | NMI_edge) ? (P & 8'b1110_1111) : P;

        default: data_out = regfile;
    endcase
end
...
/*
 * IR register/mux. Hold previous data_in value in IRHOLD in PULL0 and PUSH0
...
assign DIMUX = ~RDY ? DIHOLD : data_in;
